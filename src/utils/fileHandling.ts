...
const moduleName: string = __filename.slice(__dirname.length+1, __filename.length - 3);
...
export const unzipFile = (DATA_DIR: string, zipFileName:string) => {
    ...
    log(`${logAppend} Into unzipFile. Unzipping file: [${filePath}] to path: [${DATA_DIR}]`, 'debug');
    ...
    const dest = createWriteStream(path.join(DATA_DIR, zipFileName.substring(0, zipFileName.length-4) ) ) ;
    entry.pipe(dest);
